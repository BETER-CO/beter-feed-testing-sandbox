...
public class PlaybackScheduler : IPlaybackScheduler
{
    ...
    private async Task HandleMessagesAsync(CancellationToken cancellationToken)
    {
        ...
        {
            var playback = _playbacksRepository.RemoveMessageFromPlayback(message.PlaybackId, message);

            var actionBlock = _actionBlocks.GetOrAdd(
                message.PlaybackId,
                CreatExecutionBlock(cancellationToken));

            await actionBlock.SendAsync((playback, message));
        }
    }

    private ActionBlock<(Playback, PlaybackItem)> CreatExecutionBlock(CancellationToken cancellationToken)
    {
        return new ActionBlock<(Playback, PlaybackItem)>(
                async message =>
                {
                    var playback = message.Item1;
                    var playbackItem = message.Item2;

                    try
                    {
                        var handler = _messageHandlerResolver.Resolve(playbackItem.Message.MessageType);

                        await handler.Handle(playbackItem.Message, playbackItem.PlaybackId, playback.AdditionInfo, cancellationToken);

                        _logger.LogInformation($"{playbackItem.InternalId} published at: {DateTimeOffset.UtcNow}");
                    }
                    catch (Exception e)
                    {
